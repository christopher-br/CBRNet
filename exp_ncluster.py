...
for n in tqdm(range(n_iter), leave=False, desc='Iteration'):
    ...
    for ncluster in tqdm(iter_ncluster, desc='Iterate over nclusters', leave=False):
        # Params for tuning
        params = {
            # System parameters
            'verbose': [verbose],
        ...
        mise_name = str('MISE n:') + str(ncluster)
        pe_name = str('PE n:') + str(ncluster)
        fe_name = str('FE n:') + str(ncluster)
