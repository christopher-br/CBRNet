...
def gen_dataset(args, x, n_clusters=3):
    """
    ...
    args entries:
        'dosageSelectionBias' (float): The amount of selection bias [0, inf)
        'dosageDistributionBias' (float): How far dosages are spread apart from each other [0,1] with 0: Distributions are fully overlapping and 1: Dosages are maximally apart from each other
...
def generate_observation(x, v, distribution_bias, noise_std, response, cluster_mode):
    """
    ...
    """
    # Save alpha (1+bias)
    a = 1 + distribution_bias
    
    # Calculate beta
    b = beta_mode(a, mode=cluster_mode)
