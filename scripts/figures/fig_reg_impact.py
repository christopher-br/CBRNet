...
means = (
    data.groupby(["reg_strength"])
    .mean()
    .round(2)
    .sort_values("reg_strength", ascending=True)
    .transpose()
    .sort_index()
)
...
ax1.set_xlabel('Reg. strength', fontsize=20)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)

# ax1.set_ylim(bottom=0)
ax1.set_ylim(top=1.5, bottom=0.5)

ax1.set_xscale('log')

fig.set_size_inches(w,h)

plt.savefig("reg_strength_impact.pdf")
...
fig_leg.savefig("reg_strength_impact_legend.pdf")
